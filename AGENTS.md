...
- Global navigation exposes four pages — Start / Home, Unternehmen / Company, Lösungen / Solutions, Kontakt / Contact — then the portal entry (Login), then the locale switch, at every width and in that order; the points-only signet leads to the homepage as well, carrying an `sr-only` name rather than a visible word. The menu contains no anchors. The portal entry is a bordered control rather than a fifth word in the row, because it is the only entry that leads out of the public site into something that asks who you are; it is secondary, never filled, so the blue current-page underline stays the header's one accent.
