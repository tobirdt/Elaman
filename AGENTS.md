...
- Global navigation exposes five entries — Start / Home, Unternehmen / Company, Lösungen / Solutions, Kontakt / Contact, Login — then the locale switch, at every width and in that order; the points-only signet leads to the homepage as well, carrying an `sr-only` name rather than a visible word. The menu contains no anchors. The portal entry is set exactly like the other four: same type, same current-page underline on desktop, same numbered row on mobile. It leads somewhere different in kind, and the menu does not say so in a different shape.
