...
export function TrustSection() {
  return (
    <Section id="experience" className="border-y border-line bg-porcelain/70">
      <Container className="grid gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
        <div>
          <SectionLabel>{trustContent.label}</SectionLabel>
          <h2 className="max-w-3xl text-5xl font-semibold leading-[1.03] tracking-[-0.045em] text-graphite md:text-6xl">
            {trustContent.title}
          </h2>
        ...
        <GlassPanel className="glass-surface-strong overflow-hidden p-6 sm:p-8">
          <div className="grid gap-px overflow-hidden rounded-lg border border-line bg-line md:grid-cols-3">
            {trustContent.metrics.map((metric, index) => (
              <div key={metric.value} className="bg-white/78 p-5">
                ...
                <p className="mt-8 text-4xl font-semibold tracking-[-0.055em] text-graphite">
                  {metric.value}
                </p>
