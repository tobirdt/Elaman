      ...
      <Container>
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {capabilityOverview.items.map((item, index) => (
            <article
              key={item.title}
              className="group relative overflow-hidden rounded-lg border border-line bg-white p-6 shadow-[0_24px_80px_rgba(22,24,29,0.045)] transition duration-200 hover:-translate-y-1 hover:border-elaman-blue/22 sm:min-h-72 lg:min-h-80"
            >
              ...
              <p className="mt-10 text-xs font-semibold text-elaman-red">
                {item.eyebrow}
              </p>
              <h3 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-graphite">
                {item.title}
              </h3>
